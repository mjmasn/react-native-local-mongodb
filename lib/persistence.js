...
Persistence.prototype.treatRawStream = function(rawStream, cb) {
  ...
  rawStream.onData(function(chunk) {
    processing += 1;
    if (chunk.indexOf('\n') > -1) {
      var lines = chunk.split('\n');
      lines.forEach((theLine, index) => {
        if (index === 0) {
          processLine(line + theLine);
  ...
  rawStream.onEnd(function() {
    // A bit lenient on corruption
    var err;
    if (length > 0 && corruptItems / length > that.corruptAlertThreshold) {
      err = new Error(
        'More than ' +
          Math.floor(100 * that.corruptAlertThreshold) +
  ...
  rawStream.onError(function(err) {
    cb(err);
  });
};
      ...
      function(cb) {
        Persistence.ensureDirectoryExists(self.filename, function(err) {
          storage.ensureDatafileIntegrity(self.filename, function(err) {
            var fileStream = storage.readFileStream(
              self.filename,
              'utf8',
              (err, fileStream) => {
                if (err) {
                ...
                self.treatRawStream(fileStream, function(err2, treatedData) {
                  if (err2) {
                    return cb(err2);
                  }
